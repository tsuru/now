...
function install_vulcand {
    ...
    docker run -d --net=host --restart=always --name tsuru_vulcand mailgun/vulcand:v0.8.0-beta.2 /go/bin/vulcand -apiInterface="0.0.0.0" -etcd=http://127.0.0.1:4001 -apiPort=8182 -apiInterface=127.0.0.1 -port=80
    router="vulcand"
}
...
function add_as_docker_node {
    ...
    status=1
    while [ $status != 0 ]; do
        tsuru-admin docker-node-list | grep "| http://" | grep waiting | wc -l | grep -q "${amount}$"
        status=$?
    done
    set -e
}
